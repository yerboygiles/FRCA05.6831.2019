...
public class driveTrain extends subsystem{
	...
	public void update() {
		...
		switch(gear) {
		case LOWGEAR:
			//SmartDashboard.putString("DB/String 4", Double.toString(motorLeft.getSelectedSensorPosition()));
			SmartDashboard.putString("DB/String 3", "Low");
			if(Math.abs(stick1.getRawAxis(1)) > .4) {
				gear = subsystemState.UPSHIFTING;
			}
		...
		case HIGHGEAR:
			...
			if(Math.abs(stick1.getRawAxis(1)) < .3) {
				gear = subsystemState.DOWNSHIFTING;
			}
	...
	int shiftstate = 0;
	subsystemState gear = subsystemState.LOWGEAR;
	double inittime = 0;
	...
	public driveTrain(Timer time, Joystick stick1, int portr1, int portr2, int portr3, int portl1, int portl2, int portl3, int ports1, int ports2) {
		this.gear = subsystemState.LOWGEAR;
		this.time = time;
		this.stick1 = stick1;
		this.portr1 = portr1;
		this.portr2 = portr2;
