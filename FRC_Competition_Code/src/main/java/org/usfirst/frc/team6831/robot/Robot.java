...
public class Robot extends TimedRobot {
	...
	public driveTrain dt = new driveTrain(time, stick1, 1, 2, 3, 8, 7, 6, 0, 1);
	public sweeper sweeper = new sweeper(stick1, 4);
	...
	public void autonomousPeriodic() {
		...
		case QUEUED:
		command.state = commandState.INITIALIZING;
			break;
		case INITIALIZING:
			if(command.init(this)) {
				command.state = commandState.UPDATING;
			}
			break;
		case UPDATING:
			if(command.update(this)) {
				command.state = commandState.RESOLVING;
			}
			break;
		case RESOLVING:
			if(command.resolve(this)){
